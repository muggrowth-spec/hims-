...
(function () {
  ...
  /* ---- Fail-safe: see main.js. If nothing has revealed after 3s, assume the
     reveal system is broken and stop the CSS from hiding content. ---- */
  setTimeout(function () {
    try {
  ...
  const team = [
    ["founder-pawan.jpg", "Founder", "Mr. Pawan Agarwal", "The founder of Him Herbal Healthcare — whose vision, and two decades of building trust with partners across the globe, is the foundation everything here stands on."],
  ...
  // If a portrait is missing, fall back to the person's initials rather than
  // rendering a broken-image icon.
  const initials = (name) => name.replace(/^(Mr|Mrs|Ms|Dr)\.?\s+/i, "").split(/\s+/).map((w) => w[0]).join("").slice(0, 2).toUpperCase();
  $("#teamGrid").innerHTML = team.map(([img, role, name, bio]) =>
    `<article class="team-card reveal"><div class="team-avatar has-img" data-initials="${initials(name)}"><img src="assets/img/${img}" alt="${name}" class="ph mono" loading="lazy" onerror="this.remove();this.parentNode.classList.add('no-img')"></div><h3>${name}</h3><span class="team-role">${role}</span><p class="team-bio">${bio}</p></article>`
  ).join("");
  ...
  $("#pressGrid").innerHTML = press.map(([img, quote, src], i) =>
    `<article class="press-card reveal mono-card" data-type="press" data-idx="${i}" tabindex="0" role="button" aria-label="Read the coverage in ${src}"><div class="press-logo has-img"><img src="assets/img/${img}" alt="${src}" class="ph mono" loading="lazy"></div><div><p>${quote}</p><span class="press-src">${src}</span><span class="press-cta">Read coverage
       <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg></span></div></article>`
  ).join("");
  ...
  const activateItem = (el) => {
    if (el.dataset.type === "photo") {
      ...
      openLb(press.map(([img, quote, src]) => ({ img: "assets/img/" + img, label: src, sub: quote })), +el.dataset.idx);
